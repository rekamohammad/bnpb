...
class ProvinsiDataTable extends DataTableAbstract
{
    /**
    ...
    public function ajax()
    {
        $data = $this->datatables
            ...
            ->editColumn('name', function ($item) {
                return anchor_link(route('provinsi.edit', $item->id), $item->name);
            })
        ...
        return apply_filters(BASE_FILTER_GET_LIST_DATA, $data, POST_MODULE_SCREEN_NAME)
            ->addColumn('operations', function ($item) {
                return table_actions('provinsi.edit', 'provinsi.delete', $item);
            })
            ->escapeColumns([])
            ->make(true);
    }
    ...
    {
        $model = app(ProvinsiInterface::class)->getModel();

        /**
         * @var \Eloquent $model
         */

        $query = $model->select(['links.id', 'provinsi.name', 'links.created_at', 'links.status'])->join('provinsi','links.province','=','provinsi.id')->where('links.categories', '=', 'Provinsi');
    ...
    public function actions()
    {
        return [
            'delete' => [
                'link' => route('provinsi.delete.many'),
                'text' => view('bases::elements.tables.actions.delete')->render(),
            ],
            'activate' => [
                'link' => route('provinsi.change.status', ['status' => 1]),
                'text' => view('bases::elements.tables.actions.activate')->render(),
            ],
            'deactivate' => [
                'link' => route('provinsi.change.status', ['status' => 0]),
