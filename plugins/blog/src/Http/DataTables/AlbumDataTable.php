...
class AlbumDataTable extends DataTableAbstract
{
    /**
    ...
    public function ajax()
    {
        $data = $this->datatables
            ...
            ->editColumn('name', function ($item) {
                return anchor_link(route('album.edit', $item->id), $item->name);
            })
        ...
        return apply_filters(BASE_FILTER_GET_LIST_DATA, $data, POST_MODULE_SCREEN_NAME)
            ->addColumn('operations', function ($item) {
                return table_actions('album.edit', 'album.delete', $item);
            })
            ->escapeColumns([])
            ->make(true);
    }
    ...
    {
        $model = app(AlbumInterface::class)->getModel();

        /**
         * @var \Eloquent $model
         */

        $query = $model->select(['album.id', 'album.name', 'album.created_at', 'album.status']);
