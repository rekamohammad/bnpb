...
class InternasionalDataTable extends DataTableAbstract
{
    /**
    ...
    public function ajax()
    {
        $data = $this->datatables
            ...
            ->editColumn('name', function ($item) {
                return anchor_link(route('internasional.edit', $item->id), $item->name);
            })
        ...
        return apply_filters(BASE_FILTER_GET_LIST_DATA, $data, POST_MODULE_SCREEN_NAME)
            ->addColumn('operations', function ($item) {
                return table_actions('internasional.edit', 'internasional.delete', $item);
            })
            ->escapeColumns([])
            ->make(true);
    }
    ...
    {
        $model = app(InternasionalInterface::class)->getModel();

        /**
         * @var \Eloquent $model
         */

        $query = $model->select(['links.id', 'links.name', 'links.created_at', 'links.status'])->where('links.categories', '=', 'Internasional');
    ...
    public function buttons()
    {
        $buttons = [
          'create' => [
              'link' => route('internasional.create'),
    ...
    public function actions()
    {
        return [
            'delete' => [
                'link' => route('internasional.delete.many'),
                'text' => view('bases::elements.tables.actions.delete')->render(),
            ],
            'activate' => [
                'link' => route('internasional.change.status', ['status' => 1]),
                'text' => view('bases::elements.tables.actions.activate')->render(),
            ],
            'deactivate' => [
                'link' => route('internasional.change.status', ['status' => 0]),
