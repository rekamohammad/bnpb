...
class NewsDataTable extends DataTableAbstract
{
    /**
    ...
    public function ajax()
    {
        $data = $this->datatables
            ...
            ->editColumn('name', function ($item) {
                return anchor_link(route('news.edit', $item->id), $item->name);
            })
        ...
        return apply_filters(BASE_FILTER_GET_LIST_DATA, $data, POST_MODULE_SCREEN_NAME)
            ->addColumn('operations', function ($item) {
                return table_actions('news.edit', 'news.delete', $item);
            })
            ->escapeColumns([])
            ->make(true);
    }
    ...
    public function query()
    {
         ...
         * @var \Eloquent $model
         */
        $news_id = get_berita()[0]['attributes']['id'];
