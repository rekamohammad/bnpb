...
class KebencanaanDataTable extends DataTableAbstract
{
    /**
    ...
    public function ajax()
    {
        $data = $this->datatables
            ...
            ->editColumn('name', function ($item) {
                return anchor_link(route('kebencanaan.edit', $item->id), $item->name);
            })
        ...
        return apply_filters(BASE_FILTER_GET_LIST_DATA, $data, POST_MODULE_SCREEN_NAME)
            ->addColumn('operations', function ($item) {
                return table_actions('kebencanaan.edit', 'kebencanaan.delete', $item);
            })
            ->escapeColumns([])
            ->make(true);
    }
    ...
    public function query()
    {
         ...
         * @var \Eloquent $model
         */
        $query = $model->orderBy('kebencanaan.created_at', 'desc')
                ->select(['kebencanaan.id', 'kebencanaan.name', 'kebencanaan.created_at', 'kebencanaan.status']);
    ...
    public function columns()
    {
        return [
            ...
            'created_at' => [
                'name' => 'kebencanaan.created_at',
                'title' => trans('bases::tables.created_at'),
                'width' => '100px',
                'class' => 'searchable',
            ],
            'status' => [
                'name' => 'kebencanaan.status',
    ...
    public function actions()
    {
        return [
            'delete' => [
                'link' => route('kebencanaan.delete.many'),
                'text' => view('bases::elements.tables.actions.delete')->render(),
            ],
            'activate' => [
                'link' => route('kebencanaan.change.status', ['status' => 1]),
                'text' => view('bases::elements.tables.actions.activate')->render(),
            ],
            'deactivate' => [
                'link' => route('kebencanaan.change.status', ['status' => 0]),
